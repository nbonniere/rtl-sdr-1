...
int r82xx_set_if_bw_regs(struct r82xx_priv *priv, uint8_t reg_0a, uint8_t reg_0b) 
{
	...
	rc = r82xx_write_reg_mask(priv, 0x0b, reg_0b, 0xff);
	if (rc < 0)
		return rc;
	return 0;
}
...
int r82xx_set_bandwidth(struct r82xx_priv *priv, uint32_t bw)
{
	int rc;
	unsigned int i;
	int real_bw = 0;
